...
class EnfNet(nn.Module):
    ...
    # Initialize
    def __init__(self, num_input_channels=19, num_latent_dims=64, num_classes=3, variant_key=2, train_key=1, flow_depth=10, flow_key=0):
        ...
        if self.variant is "AE":
            ...
            self.nf_bottleneck = NFBottleneck(num_latent_dims, flow_depth, flow_dict[flow_key]) 
    ...
    def forward(self, X, mode, device):
        ...
        if mode is "sample":
            ...
            if self.variant is "VAE":
                ...
                z = self.vae_bottleneck(None, mode, device)
                z_k = self.nf_bottleneck(z, mode)
        ...
        #  Forward pass using different modes
            ...
            if mode is "generate_latents":
                ...
                if self.variant is "AE":
                    ...
                    return self.decoder(z_k, self.encoder.unflat_size), z_k, mu, logvar, z_prime, log_det, self.classifier(z_k), self.regressor(z_k)
                
# NFBottleneck - Normalizing flow bottleneck
class NFBottleneck(nn.Module):
   
    # Initializer
    def __init__(self, num_latent_dims, flow_depth, flow_type):
    ...
    def forward(self, X, mode=None):
        ...
        log_det_jacobians = zeros(self.flow_depth, z.size(0))
