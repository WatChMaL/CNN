        ...
        for i in np.arange(len(self.datasets)):
            ...
            if index < self.labels[self.datasets[i]].shape[0]:
                ...
                if self.a.shape[0] == 16:
                    ...
                    # os.posix_fadvise(self.fds[i].fileno(), self.offsets[i], self.dataset_sizes[i], 
                    #                                      os.POSIX_FADV_DONTNEED)
                    return np.squeeze(self.chrg_func(np.expand_dims(np.ascontiguousarray(np.transpose(data,[2,0,1])), axis=0), self.chrg_acc, apply=True)), self.labels[self.datasets[i]][index], self.energies[self.datasets[i]][index], self.angles[self.datasets[i]][index], index, self.positions[self.datasets[i]][index]
