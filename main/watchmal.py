...
_ARGS = [Argument('model', list, '-mdl', list_dtype=str, default=['resnet', 'resnet18'], help='Specify model architecture. Default = resnet resnet18.'),
         Argument('params', list, '-pms', list_dtype=str, default=None, help='Soecify model constructer args. Default = None'),
         Argument('device', str, '-dev', default='cpu', help='Choose either \'cpu\' or \'gpu\'. Default = \'cpu\''),
         Argument('gpu_list', list, '-gpu', list_dtype=int, default=0, help='Indices of the device GPUs to utilize. E.g. 0 1. Default = 0.'),
         Argument('path', str, '-pth', default=None, help='Absolute path for the training dataset. Default = None'),
         Argument('subset', int, '-sbt', default=None, help='Number of samples to use from the entire dataset. Default = None'),
         Argument('shuffle', bool, '-shf', default=True, help='Specify whether or not to shuffle the dataset. Default = True.'),
         Argument('cl_ratio', float, '-clr', default=0.1, help='Fraction of dataset to be used for classifier training and validation. Default = 0.1'),
         Argument('val_split', float, '-vst', default=0.1, help='Fraction of entire dataset to be used for validation. Default = 0.1'),
         Argument('test_split', float, '-tst', default=0.1, help='Fraction of dataset to be used for testing. Default = 0.1'),
         ...
         Argument('tasks', list, list_dtype=str, flag='-d', default=['train', 'test', 'valid'], help='Specify list of tasks to be performed in this run.'),
         Argument('dump_path', str, '-dmp', default='dumps/', help='Specify path to dump data to. Default is dumps.'),
         ...
         Argument('cfg', str, '-sav', default=None, help='Specify name for destination config file. No action by default.'),
         Argument('githash', str, '-git', default=None, help='git-hash for the latest commit'),
         Argument('report_interval', int, '-rpt', default=10, help='Interval at which to report the training metrics to the user'),
         Argument('num_vals', int, '-nvl', default=1000, help='Number of validations to perform during the entire training'),
         Argument('num_val_batches', int, '-nvb', default=16, help='Number of batches to use for each validation during training'),
         Argument('num_dump_events', int,'-nde', default=128, help='Number of events and their results to dump to a .npz file'),
